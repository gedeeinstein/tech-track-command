...
export const createAsset = async (asset: Omit<Asset, "id" | "inventoryNumber">): Promise<Asset> => {
  try {
    ...
    if (!asset.division) {
      console.warn("No department ID provided for asset, using default GEN");
    }
    
    // Generate inventory number using the department ID directly
    const inventoryNumber = await generateInventoryNumber(
      asset.type, 
      asset.division || ""
    );
    
    console.log("Generated inventory number:", inventoryNumber);
